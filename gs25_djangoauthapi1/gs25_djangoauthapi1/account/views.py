...
class UserRegistrationView(APIView):
    ...
    def post(self, request, format = None):
        ...
        if serializer.is_valid(raise_exception = True):
            ...
            return Response({'token':token, 'mgs': 'Registration Success '}, status = status.HTTP_201_CREATED)
...
class UserLoginView(APIView):
    ...
    def post(self, request, format = None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if user is not None:
                token = get_tokens_for_user(user)
                return Response({'token':token, 'mgs': 'Login Successfull'}, status = status.HTTP_200_OK)
            else:
                return Response({'errors' : {'non_field_errors': ['Email or Password is not Valiod']}},
...
class UserChangePassword(APIView):
    ...
    def post(self, request, format = None):
        ...
        if serializer.is_valid(raise_exception=True):
            return Response({'mgs':'Password Change Successfull'}, status=status.HTTP_200_OK)
        return Response(serializer.errors, status = status.HTTP_400_BAD_REQUEST)
